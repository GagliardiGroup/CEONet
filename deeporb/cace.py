...
class Cace(nn.Module):
    ...
    def __init__(
        ...
        self.angular_basis = AngularComponent_GPU(self.max_l) #use gpu ver.
    ...
    def forward(
        ...
        if data["batch"] == None:
            batch_now = torch.zeros(n_nodes, dtype=torch.int64).to(data['atomic_numbers'].device)
        else:
