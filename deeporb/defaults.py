...
PARSER = get_parser()
argnames = [PARSER._actions[i].dest for i in range(len(PARSER._actions))]
defaults = [PARSER._actions[i].default for i in range(len(PARSER._actions))]
arg_to_default = dict(zip(argnames,defaults))
