...
class OrbData(L.LightningDataModule):
    def __init__(self, root="data/aodata.h5", cutoff=4.0, in_memory=False, inmem_parallel=False, drop_last=True,
                 batch_size=32, num_train=None, num_val=None, shuffle=True, valid_p=0.1, test_p=0.1, avge0=0, sigma=1):
        super().__init__()
    ...
    def prepare_data(self):
        ...
        if self.num_val:
            ...
            self.test = self.test[:self.num_val]
            assert(self.num_val == len(self.test))
